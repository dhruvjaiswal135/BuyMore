...
import { IoMdArrowDropright, IoMdImages } from "react-icons/io";
...
const AddProduct = () => {
        ...
        <div className="w-full flex flex-wrap mt-5 justify-between g">
          ...
          <div className="w-full lg:w-5/12 lg:pl-4 mt-5 lg:mt-0  ">
            <div className="p-4 shadow-md rounded-md border mb-5 border-gray-50">
              <div>
                <h1>Product Media</h1>
                <h2 className="text-xs mt-3 text-gray-400">Photo Product</h2>
                <div className="grid lg:grid-cols-4 grid-cols-1 md:grid-cols-3 sm:grid-cols-2
                sm:gap-4 md:gap-4 gap-3 w-full mt-1">
                  {/* Product Photo */}
                  <label
                    htmlFor="prodimage"
                    className="flex justify-center items-center flex-col
                    h-[180px] cursor-pointer border border-dashed mb-1 text-gray-500 font-normal"
                  >
                  ...
                  <input
                    ...
                    className="hidden border border-gray-100 rounded-md px-3 py-2 text-xs font-normal bg-[#f7fafb]"
                  />
                </div>
              </div>
            </div>
  ...
  <div className="flex flex-col mb-3 md:flex-row gap-4 w-full text-gray-500 text-sm">
    <div className="flex flex-col w-full gap-1 relative">
      ...
      {/* Category Dropdown */}
      <div className={`absolute top-[105%] left-0 bg-white shadow-md mt-2 rounded-md w-full transition-all duration-300 ease-in-out overflow-hidden ${cateShow ? 'scale-100 opacity-100' : 'scale-95 opacity-0 pointer-events-none'}`}>
        ...
        <div className="flex flex-col max-h-[150px] overflow-y-auto">
          {allCategory.map((c, i) => (
            ...
            >
              ...
              />
              <span className={`w-4 h-4 border-2 rounded-full transition-all duration-200 flex items-center justify-center 
                ${category === c.name ? 'border-[#3948ab]' : 'border-gray-400'}`}>
                {category === c.name && (
                  <span className="w-2.5 h-2.5 bg-[#3948ab] rounded-full"></span>
                )}
              </span>
              <span className={`text-sm ${category === c.name ? 'font-semibold' : 'font-normal'}`}>
                {c.name}
              </span>
            </label>
          ))}
